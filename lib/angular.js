...
/* ==========================================================================
   THEME TOGGLES (system + smooth flips +  (Black & White) + surface + helpers)
   ========================================================================== */
...
/* =================
   DESIGN TOKENS
   ================= */
/* LIGHT (default, monochrome) */
:root,
:root[data-theme="light"],
html[data-theme="light"] {
  /* Brand grayscale */
  --p-primary-50:#fafafa;  --p-primary-100:#f5f5f5; --p-primary-200:#e5e5e5;
  --p-primary-300:#d4d4d4; --p-primary-400:#a3a3a3; --p-primary-500:#737373;
  --p-primary-600:#525252; --p-primary-700:#404040; --p-primary-800:#262626; --p-primary-900:#171717;
  /* Surfaces & text */
  --p-surface-0:#ffffff;   --p-surface-50:#f9f9f9;  --p-surface-100:#f2f2f2; --p-surface-200:#e6e6e6;
  --p-text-color:#111111;  --p-text-color-secondary:#6f6f6f;
  /* Status (neutralized) */
  --p-success-500:#8a8a8a; --p-warning-500:#777777; --p-danger-500:#444444;
  /* Aliases */
...
/* DARK (inverted neutrals) */
:root[data-theme="dark"],
html[data-theme="dark"] {
  --p-primary-50:#0f0f0f;  --p-primary-100:#141414; --p-primary-200:#1b1b1b;
  --p-primary-300:#222222; --p-primary-400:#2b2b2b; --p-primary-500:#3a3a3a;
  --p-primary-600:#5a5a5a; --p-primary-700:#7a7a7a; --p-primary-800:#bdbdbd; --p-primary-900:#f0f0f0;
  --p-surface-0:#111111;   --p-surface-50:#141414; --p-surface-100:#1a1a1a; --p-surface-200:#232323;
  --p-text-color:#f2f2f2;  --p-text-color-secondary:#a7a7a7;
  --p-success-500:#bbbbbb; --p-warning-500:#9b9b9b; --p-danger-500:#7b7b7b;
  --primary-color: var(--p-primary-800);
  --surface-50: var(--p-surface-50);
  --surface-100: var(--p-surface-100);
  --surface-200: var(--p-surface-200);
  --accent: #d0d0d0;
}
/* Map PrimeNG legacy tokens -> our tokens (some components use these) */
:root,
:root[data-theme="light"],
:root[data-theme="dark"] {
  --text-color: var(--p-text-color);
  --text-color-secondary: var(--p-text-color-secondary);
  --surface-a: var(--p-surface-0);
  --surface-b: var(--p-surface-50, var(--p-surface-0));
  --surface-c: var(--p-surface-100);
  --surface-d: var(--p-surface-200);
}
/* =================
   BASE / LAYOUT
   ================= */
...
body, .p-component { color: var(--p-text-color); }
/* Helpers */
.container { max-width:1120px; margin-inline:auto; padding-inline:1rem; }
.section { margin-block:1rem; }
.h1 { font-size: clamp(1.5rem, 2vw + 1rem, 2.25rem); font-weight: 800; }
...
/* =================
   PRIME NG POLISH
   ================= */
/* Header surface aligns with theme */
.header { background: var(--p-surface-0); }
/* Cards & containers */
.p-card, p-card, .p-panel, .p-dataview, .p-datatable-wrapper {
  background: var(--p-surface-0);
  border: 1px solid var(--p-surface-200);
  border-radius: 12px;
}
/* Inputs / dropdowns / toggles */
.p-inputtext, .p-dropdown, .p-multiselect, .p-toggleswitch, .p-inputnumber, .p-calendar {
  background: var(--p-surface-0);
  color: var(--p-text-color);
  border: 1px solid var(--p-surface-200);
  border-radius: 12px;
}
.p-inputtext:enabled:focus, .p-dropdown:focus-within {
  outline: none;
...
.p-button {
  ...
  color: #fff;
  border: 1px solid color-mix(in oklab, var(--primary-color) 100%, #000 10%);
}
.p-button:hover {
  background: color-mix(in oklab, var(--primary-color) 92%, #000 8%);
  border-color: color-mix(in oklab, var(--primary-color) 100%, #000 16%);
}
/* Tags / pills */
.p-tag {
...
/* Menubar (text inherits correctly) */
.header .p-menubar { background: transparent; border: 0; }
.header .p-menubar .p-menuitem-text { color: var(--p-text-color); }
/* Scrollbar (dark mode subtle) */
:root[data-theme="dark"] *::-webkit-scrollbar { width: 10px; height: 10px; }
:root[data-theme="dark"] *::-webkit-scrollbar-thumb { background: #2b2b2b; border-radius: 10px; }
/* Optional: for custom tiles */
.tile, .home-card {
  background: var(--p-surface-0);
  color: var(--p-text-color);
  border: 1px solid var(--p-surface-200);
}
/* Dark-mode base background is slightly lighter than full black for contrast */
:root[data-theme="dark"] html,
:root[data-theme="dark"] body { background: var(--p-surface-50); }
`
    );
  }
