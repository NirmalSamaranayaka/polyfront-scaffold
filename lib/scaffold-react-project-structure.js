...
export const fmt = (d${useTS ? ': Date | string' : ''}, format${useTS ? ': string' : ''} = 'YYYY-MM-DD') => moment(d).format(format);
export const fromNow = (d${useTS ? ': Date | string' : ''}) => moment(d).fromNow();
export const isToday = (d${useTS ? ': Date | string' : ''}) => moment(d).isSame(moment(), 'day');
export const isYesterday = (d${useTS ? ': Date | string' : ''}) => moment(d).isSame(moment().subtract(1, 'day'), 'day');
export const formatRelative = (d${useTS ? ': Date | string' : ''}) => {
  if (isToday(d)) return 'Today';
  if (isYesterday(d)) return 'Yesterday';
  return fromNow(d);
