...
function createTailwindPages(pagesDir, extx, ext, useTS) {
  const home = path.join(pagesDir, `Home.${extx}`);
  if (!fs.existsSync(home)) {
    writeFileSafe(home, `export default function Home() {
  const features = [
  ...
  createOtherTailwindPages(pagesDir, extx, ext, useTS);
}

function createOtherTailwindPages(pagesDir, extx, ext, useTS) {
  // About page
  const about = path.join(pagesDir, `About.${extx}`);
  if (!fs.existsSync(about)) {
    writeFileSafe(about, `export default function About() {
...
function createAntdPages(pagesDir, extx, ext, useTS) {
   const home = path.join(pagesDir, `Home.${extx}`);
  if (!fs.existsSync(home)) {
    writeFileSafe(home, `import { Typography, Row, Col, Card, Button, Avatar, theme } from 'antd';
import {
  RiseOutlined,
  TeamOutlined,
  ThunderboltOutlined,
...
`);
  ...
  // Create other Antd pages
  createOtherAntdPages(pagesDir, extx, ext, useTS);
}

function createOtherAntdPages(pagesDir, extx, ext, useTS) {
   // About page
  const about = path.join(pagesDir, `About.${extx}`);
  if (!fs.existsSync(about)) {
    writeFileSafe(about, `import { Typography, Row, Col, Card, List, Divider, Tag } from 'antd';
...
function createChakraPages(pagesDir, extx, ext, useTS) {
   const home = path.join(pagesDir, `Home.${extx}`);
  if (!fs.existsSync(home)) {
    writeFileSafe(home, `import { Box, Text, SimpleGrid, Container, Card, Heading, Avatar, Button, Stack } from '@chakra-ui/react';
...
`);
  ...
  // Create other Antd pages
  createOtherChakraPages(pagesDir, extx, ext, useTS);
}

function createOtherChakraPages(pagesDir, extx, ext, useTS) {
   // About page
  const about = path.join(pagesDir, `About.${extx}`);
  if (!fs.existsSync(about)) {
    writeFileSafe(about, `import { Box, Text, Container, Card, Heading, SimpleGrid, Tag, List, Separator, Icon, HStack } from '@chakra-ui/react';
...
export default function About() {
  ...
  const dashboard = path.join(pagesDir, `Dashboard.${extx}`);
  if (!fs.existsSync(dashboard)) {
    writeFileSafe(dashboard, `import {
      Box,
      Heading,
      Text,
      SimpleGrid,
    ...
    export default function Dashboard() {
      ...
      const recentActivity = [
        { id: 1, user: 'Nirmal Samaranayaka', action: 'Order placed', time: '2 min ago', status: 'success' as const },
        { id: 2, user: 'Jane Smith', action: 'Payment received', time: '5 min ago', status: 'success' as const },
        { id: 3, user: 'Bob Johnson', action: 'Login attempt', time: '10 min ago', status: 'warning' as const },
        { id: 4, user: 'Alice Brown', action: 'Account created', time: '15 min ago', status: 'success' as const },
      ];
      ...
      <AppBar position="fixed" sx={{ zIndex: theme.zIndex.drawer + 1 }}>
        <Toolbar>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Chip label="v1.0.6" size="small" variant="outlined" />
        ...
        <header className="bg-white border-bottom p-3">
          <div className="d-flex justify-content-between align-items-center">
            ...
            <div className="d-flex align-items-center gap-2">
              <span className="badge bg-secondary">v1.0.6</span>
            ...
            <div className="flex-1">
              ...
              <header className="bg-white border-b border-gray-200 p-4 flex justify-between items-center">
                ...
                <div className="flex items-center gap-3">
                  {/* Version */}
                  <span className="bg-gray-100 text-gray-700 px-2 py-1 rounded-full text-sm">v1.0.6</span>
