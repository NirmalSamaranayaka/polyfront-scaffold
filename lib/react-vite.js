...
function setupReactVite(projectName, useTS, ui, args, pmCfg, log) {
  ...
  const core = ["react-router-dom", "@tanstack/react-query", "axios", "zod", "zustand", "clsx", "react", "react-dom"];
  let uiPkgs = [];
  ...
  else if (ui === "chakra") uiPkgs = ["@chakra-ui/react", "@emotion/react", "@emotion/styled", "framer-motion", "react-icons","next-themes"];
  ...
  if (ui === "bootstrap") {
    additionalPkgs.push("bootstrap-icons");
  } else if (ui === "tailwind") {
    additionalPkgs.push("@tailwindcss/forms", "@tailwindcss/typography");
  } else if (ui === "mui") {
    additionalPkgs.push("@mui/lab", "@mui/x-date-pickers");
  }
  
  // Add common utility packages
  additionalPkgs.push("lodash-es", "date-fns");

  const devBase = ["eslint", "eslint-plugin-react", "eslint-plugin-react-hooks", "eslint-config-prettier", "prettier", "vitest", "jsdom", "@testing-library/react", "@testing-library/jest-dom"];
  const devTS = useTS ? ["@typescript-eslint/eslint-plugin", "@typescript-eslint/parser", "vite-tsconfig-paths", "@types/node"] : [];
