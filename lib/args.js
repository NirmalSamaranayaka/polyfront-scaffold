...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    if (a.startsWith("--")) {
      if (a.includes("=")) {
        ...
        args[k] = coerce(rest.join("="));
      } else {
        const k = a.slice(2);
        const next = argv[i + 1];
        if (next && !next.startsWith("--")) { args[k] = coerce(next); i++; }
        else { args[k] = true; }
      }
    } else {
      args._.push(a);
  ...
  if (args.js || args.jsx) args.lang = "js";
  ...
  if (!args["on-exists"]) args["on-exists"] = "prompt";
  if (!args["test-unit"]) args["test-unit"] =
    String(args.framework).toLowerCase() === "react-webpack" ? "jest" : "vitest";
